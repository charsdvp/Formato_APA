...
export default function getDataFormThesis() {
  ...
  const initialsAuthorThesis = document.getElementById(
    "initials-author-thesis"
  );
  const yearPublicationThesis = document.getElementById(
    "year-publication-thesis"
  );
  const titleThesis = document.getElementById("title-thesis");
  const typeThesis = document.getElementById("type-thesis");
  const nameInstitutionThesis = document.getElementById(
    "name-intitution-thesis"
  );
  const ubicationInstitutionThesis = document.getElementById(
    "ubication-intitution-thesis"
  );
  ...
  const initialsFormated = formatFullName(initialsAuthorThesis.value); // ( X. X. )
  exampleThesis.textContent = `${lastNameThesis.value}, ${initialsFormated} (${yearPublicationThesis.value}). ${titleThesis.value} (Tesis de ${typeThesis.value}). ${nameInstitutionThesis.value}, ${ubicationInstitutionThesis.value}`;
}
