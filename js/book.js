...
export default function getDataFormBook(){        
  // capturamos los campos requeridos
   ...
   const placePublicationBook = document.getElementById('place-publication-book')
   const statePublicationBook = document.getElementById('state-publication-book')
   ...
   exampleBook.textContent = `${lastNameBook.value}, ${initialsAuthorBook.value} (${yearPublicationBook.value}). ${titleBook.value} (pp. ${numPagesBook.value}). ${placePublicationBook.value}, ${statePublicationBook.value}: ${namePublisherBook.value}`
 }
