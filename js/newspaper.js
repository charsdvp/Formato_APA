...
export default function getDataFormNewsPaper(){        
  // capturamos los campos requeridos
   const lastNameNewsPaper = document.getElementById('lastname-author-newspaper')
   const initialsAuthorNewsPaper = document.getElementById('initials-author-newspaper')
   const datePublicationNewsPaper = document.getElementById('date-publication-newspaper')
   const titleArticleNewsPaper = document.getElementById('title-article-newspaper')
   ...
   // Smith, J. A. (2022, 10 de mayo). Avances en la lucha contra el cambio climático. El Periódico, pp. A1, A4.
    exampleNewsPaper.textContent = `${lastNameNewsPaper.value}, ${initialsAuthorNewsPaper.value} (${datePublicationNewsPaper.value}). ${titleArticleNewsPaper.value}. ${nameNewsPaper.value}, pp. ${pagesNewsPaper.value}.`
 }
